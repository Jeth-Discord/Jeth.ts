...
module.exports = class Ban extends Command {
    ...
    async run(message, args) {
        ...
        if (message.member.guild.me.highestRole.comparePositionTo(role) < 0) {
...
var f = async (membros, i = 0, role, message) => {
    if (i % 20 === 0) {
        await message.edit(`${i}/${message.member.guild.memberCount} **Removendo o cargo ${role} de todos os usuários do servidor**`).catch(() => { });
    }
    if (membros[i]) {
