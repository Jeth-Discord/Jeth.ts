        ...
        switch (args[0]) {
            case "add": {
                ...
                const strike = new MessageEmbed()
                    ...
                    .setFooter("https://discordapp.com/guidelines・Discord da Jeth", message.member.guild.iconURL({ dynamic: true, size: 1024 }))
                    .setTimestamp(new Date());
                userDB.strike += 1
                userDB.save().then(() => message.channel.createMessage(strike))
                if (userDB.strike >= 3) {
                    userDB.blacklist = true
                    userDB.blacklistReason = "[STRIKE EXCEDIDO] - Quebra dos termos de uso do Discord."
                    userDB.save().then(() => message.channel.createMessage(`O usuário \`${user.tag}\` foi adicionado da blacklist.`))
                }
                message.channel.createMessage(`O usuário \`${user.tag}\` acabou levando strike (${userDB.strike}))`)
            ...
            case "remove": {
                let user = await this.client.users.fetch(args[1])
                let userDB = await this.client.database.Users.findById(user.id)
                message.channel.createMessage(`${user.tag} Teve seu strike removido! <:9605discordslashcommand:832746522865172530>`)
                userDB.strike -= 1
                userDB.save()
                if (userDB.strike < 3) {
                    userDB.blacklist = false
                    userDB.blacklistReason = null
                    userDB.save().then(() => message.channel.createMessage(`O usuário \`${user.tag}\` foi removido da blacklist.`))
                }
                message.channel.createMessage(`O usuário \`${user.tag}\` teve 1 strike removido (${userDB.strike})`)
            }
                break
            default: {
                message.channel.createMessage("Você não informou a opção e o ID do usuário que deseja dar strike.")
            }
        }
    }
}