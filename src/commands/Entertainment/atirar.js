...
module.exports = class atirar extends Command {
  ...
  async run(message, args) {
    ...
    if (!user) return message.channel.createMessage('Mencione alguém');
    if (user.user.id == message.author.id) return message.channel.createMessage('Você não pode atirar em si mesmo');
    ...
    try {
      ...
      message.channel.createMessage(aEmbed);
    } catch (error) {
      console.log(error);
