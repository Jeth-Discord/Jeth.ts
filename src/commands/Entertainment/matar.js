...
module.exports = class matar extends Command {
  ...
  async run(message, args) {
    if (!message.channel.nsfw) return message.channel.createMessage("<:CancelarK:673592197341249559> Este comando só pode ser executado em um canal **NSFW**")
    let slapUser = message.member.guild.member(message.mentions.users.first() || message.member.guild.members.get(args[0]));
    if (!slapUser) return message.channel.createMessage("`Mencione o usuário que deseje matar!`");
    ...
    let killEmbed = new Discord.MessageEmbed()
      ...
      .setFooter(`Killstreak por ${message.author.username}`, message.author.avatarURL)

    message.channel.createMessage(killEmbed)
    try {
