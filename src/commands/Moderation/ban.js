...
module.exports = class Ban extends Command {
  ...
  async run(message, args) {
    ...
    // ban padrão 17
    let membro17 = message.mentions.members.first() || message.member.guild.members.get(args[1]);
    ...
    // banimento private
    let bans = await message.member.guild.fetchBans('716845015678320661');
    let membro14 = message.mentions.members.first() || message.member.guild.members.get(args[1]);
    if (!membro14) return message.reply("eu procurei, procurei, e não achei este usuário")
    let bannable = message.member.guild.member(membro17, membro14)
    if (bannable) {
    ...
    const warnembed18 = new Discord.MessageEmbed()
      ...
      .setDescription(`:do_not_litter: **Você foi banido do servidor ${message.member.guild.name} :no_entry_sign:**`)
      .setColor("#ffefad")
    ...
    if (argumentos) {
      if (!membro14.bannable) {
        message.channel.createMessage(`Erro ${Error}`)
      } else {
        message.member.guild.members.ban(membro14)
      ...
      message.channel.createMessage(escolha).then(m => {
        ...
        collector.on('collect', (reaction) => {
          ...
          switch (reaction.emoji.name) {
            ...
            case 'JT1':
              ...
              warnembed14.setDescription(`**Banido!** \n \n<:Kaeltec:673592197177933864> **Staff:** ${message.author.username} \n**ID:** ${message.author.id}` + `\n<:Kaeltec:673592197177933864> **Banido:** ${membro14.user.username} \n**ID:** ${membro14.id}` + `\n<:Registrado:673592197077270558> **Motivo:** ${reason}`)
              collector.stop()
              message.member.guild.members.ban(membro14.id, {
                reason: reason
              }).then(() => {
                message.channel.createMessage(warnembed14)
                try {
