...
module.exports = class div extends Command {
    ...
    async run(message, args) {
        ...
        if (args[0] === 'canal') {
            ...
            if (!guildDocument.antInvite) return message.channel.createMessage(`Este servidor não possui um contador ativado!`)
            guildDocument.antInvite = false
            guildDocument.infoantinv = ''
            guildDocument.save()
            message.channel.createMessage('Okay o módulo de Anti-Convite foi desativado.')
        } else if (args[0] === 'ativar') {
            if (!guildDocument.infoantinv) return message.channel.createMessage('Este servidor não tem um canal de log de invite setado. utilize' + guildDocument.prefix + 'antiinvite #canal')
            guildDocument.antInvite = true
            guildDocument.save()
            message.channel.createMessage('Okay o módulo de Anti-Convite foi Ativado.')
        } else {
